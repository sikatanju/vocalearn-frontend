...
const AllTranscriptions = () => {
    ...
    const fetchAllTranscriptions = async () => {
        ...
        try {
            ...
            const transcriptionItems: TranscriptionItem[] = response.items.map(
                (item: SavedItem) => ({
                    id: item.id,
                    audioUrl: item.content.audio_url || null,
    ...
    const handleDownloadAudio = async (audioUrl: string, id: string) => {
        try {
            // TODO: Implement backend API for audio download
            // For now, just show a placeholder behavior
            console.log('Download audio:', audioUrl, id);

            // Future implementation:
            // const response = await apiClient.get(`audio/download/${id}/`, {
            //     responseType: 'blob'
            // });
            ...
            // link.href = url;
            // link.download = `transcription-${id}.wav`;
            // link.click();
            // window.URL.revokeObjectURL(url);

            alert('Audio download feature will be implemented soon!');
        } catch (err) {
